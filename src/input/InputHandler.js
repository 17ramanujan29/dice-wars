...
export class InputHandler {
    ...
    initEventListeners() {
        ...
        window.addEventListener('touchend', (e) => this.handlePointerUp(e), { passive: false });
    }
    ...
    handlePointerMove(e) {
        if (!this.isDragging || this.game.phase !== 'playing' || !this.renderer.canDrag) return;
    ...
    handlePointerUp(e) {
        ...
        if (Math.hypot(pos.x - this.startPos.x, pos.y - this.startPos.y) < UI_CONFIG.clickTolerance) {
            ...
            if (
                tHex.col >= 0 && tHex.col < GAME_CONFIG.gridWidth &&
                tHex.row >= 0 && tHex.row < GAME_CONFIG.gridHeight
            ) {
