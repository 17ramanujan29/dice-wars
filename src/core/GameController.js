...
export class GameController {
  ...
  startGame(numPlayers, rules, playerNames = []) {
    this.rules = rules;
  ...
  areAdjacent(t1, t2) {
    return t1.hexes.some((h1) =>
      getValidNeighbors(h1.c, h1.r).some(
        (nc) => this.hexGrid[nc.col][nc.row].territoryId === t2.id,
      ),
    );
  }
  ...
  initiateBattle(source, target) {
    ...
    if (this.onBattleStart) {
      this.onBattleStart(
        source,
        target,
        this.players[source.owner],
        this.players[target.owner],
      );
    }

    setTimeout(
      () => this.resolveBattle(source, target),
  ...
  resolveBattle(source, target) {
    ...
    if (atkRoll > defRoll) {
      ...
      if (
        ...
        this.getMaxConnected(source.owner) >= this.rules.greatPowerThreshold
      ) {
  ...
  distributeReinforcements(pId) {
    ...
    if (
      this.rules.smallCountryBonus &&
      maxConnected <= this.rules.smallCountryThreshold
    ) {
      count = Math.ceil(
        GAME_CONFIG.reinforcementRatio * this.rules.smallCountryThreshold,
      );
    }
    if (
      this.rules.greatPower &&
      maxConnected >= this.rules.greatPowerThreshold
    ) {
      count = Math.ceil(
        GAME_CONFIG.reinforcementRatio * this.rules.greatPowerThreshold,
      );
    }

    const maxDice =
      this.rules.greatPower && maxConnected >= this.rules.greatPowerThreshold
  ...
  rollDice(count) {
    return Array.from(
      { length: count },
      () => Math.floor(Math.random() * GAME_CONFIG.diceSides) + 1,
    ).reduce((a, b) => a + b, 0);
  }
  ...
  applySnapshot(snapshot) {
    ...
    this.phase = snapshot.phase || "start";
